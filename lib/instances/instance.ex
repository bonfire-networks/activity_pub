defmodule ActivityPub.Instances.Instance do
  ...
  @doc "Records that we tried this host, without changing anything else about it."
  def touch(%Instance{} = record) do
    # `force: true` is what makes an empty changeset still hit the database, so `updated_at` is autogenerated at the schema's own precision rather than set by hand
    record
    |> change()
    ...
    if host do
      ...
      cond do
        ...
        existing_record.unreachable_since &&
          ...
          # keep the OLDEST `unreachable_since`, since that is what says how long the outage has run, but still record that we tried: `updated_at` is what `Instances.pacing_snooze_sec/1` reads as the last attempt, and if it froze at the first failure every queued delivery would sail past the backoff
          touch(existing_record)
