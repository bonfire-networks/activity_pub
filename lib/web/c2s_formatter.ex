defmodule ActivityPub.Web.C2SFormatter do
  @moduledoc """
  ...
  def format_activity(params, current_user) do
    case Map.get(params, "type") do
      ...
      type when type in ["Note", "Article", "Event", "Place", "Document", "Image", "Video", "Audio"] ->
  ...
  defp extract_target(%{"object" => target}), do: {:ok, target}
  defp extract_target(%{"target" => target}), do: {:ok, target}
  defp extract_target(_), do: {:error, "Missing target object"}
  ...
  defp format_object_for_post(%{"type" => "Note"} = object) do
    ...
    # Handle reply_to
    attrs = case Map.get(object, "inReplyTo") do
  ...
  defp public_addressed?(addresses) do
    Enum.any?(addresses, &(&1 == "https://www.w3.org/ns/activitystreams#Public" || &1 == "Public"))
  end
