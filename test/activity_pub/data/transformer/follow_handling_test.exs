...
defmodule ActivityPub.Federator.Transformer.FollowHandlingTest do
  ...
  describe "handle_incoming" do
    ...
    test "with locked accounts, it does create a Follow, but not an Accept" do
      ...
      accepted_i = length(list_accepts())
      ...
      assert length(accepts) == accepted_i
    ...
    test "it works for incoming follow requests from hubzilla" do
      user = local_actor(request_before_follow: false)
      ...
      {:ok, %Activity{id: id, data: data, local: false}} =
        Transformer.handle_incoming(data)
        |> debug

      # {:ok, %{data: data}} = Object.get(id: id)
    ...
    test "it works for incoming follows to locked account" do
      pending_follower = actor(ap_id: "https://mastodon.local/users/admin")
      user = local_actor(request_before_follow: true)
