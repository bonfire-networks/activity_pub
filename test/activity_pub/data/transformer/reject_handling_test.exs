...
defmodule ActivityPub.Federator.Transformer.RejectHandlingTest do
  ...
  test "it rejects incoming follow requests from blocked users " do
    ...
    {:ok, target} =
      ActivityPub.Actor.get_cached_or_fetch(ap_id: "https://mastodon.local/users/admin")
      |> debug("targettt")
    ...
    {:reject, _} = Transformer.handle_incoming(data)
    # TODO: should we actually send back a reject activity or just drop it
  ...
  test "it works for incoming rejects which are referenced by IRI only" do
    ...
    # assert requested?(follower, followed) == true
  ...
  describe "when accept/reject references a transient activity" do
    test "it handles reject activities that do not contain an ID key" do
      follower = local_actor()
      followed = local_actor(request_before_follow: false)
      {:ok, follow_activity} = follow(follower, followed)

      assert following?(follower, followed)

      without_id = Map.delete(ap_object_from_outgoing(follow_activity).data, "id")
